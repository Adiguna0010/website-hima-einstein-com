  ...
  const getRoleBadgeStyle = (roleName) => {
    switch (roleName) {
      ...
      case 'Operator External':
        return 'bg-cyan-100 text-cyan-800 border border-cyan-300 font-bold';
      case 'Anggota Hima':
      case 'Anggota Biasa':
      default:
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-xs text-slate-700">
                {(!loginLogs || loginLogs.length === 0) ? (
                  ...
                  loginLogs.slice(0, 50).map((log, idx) => {
                    const matchedUser = users.find(u => 
                      (u.email && log.email && u.email.toLowerCase().replace(/\s+/g, '') === log.email.toLowerCase().replace(/\s+/g, '')) ||
                      (u.nim && log.nim && String(u.nim).trim() === String(log.nim).trim())
                    );
                    const rawRole = matchedUser?.role || log.role || 'Anggota Hima';
                    const activeRole = rawRole === 'Anggota Biasa' ? 'Anggota Hima' : rawRole;
                        ...
                        <td className="px-4 py-3 font-bold text-slate-900">
                          <div className="flex items-center gap-2">
                            <div className="w-6 h-6 rounded-full bg-gold/15 text-gold-dark font-bold text-[10px] flex items-center justify-center">
                              {(matchedUser?.name || log.name) ? (matchedUser?.name || log.name).charAt(0).toUpperCase() : 'U'}
                            </div>
                            <div>
                              <span>{matchedUser?.name || log.name}</span>
                              <span className="block text-[10px] text-slate-400 font-normal">{matchedUser?.email || log.email}</span>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-3 font-mono text-slate-600">{matchedUser?.nim || log.nim}</td>
                        <td className="px-4 py-3">
                          <span className={`px-2.5 py-0.5 rounded text-[10px] font-bold ${getRoleBadgeStyle(activeRole)}`}>
                            {activeRole}
                          </span>
                        </td>
